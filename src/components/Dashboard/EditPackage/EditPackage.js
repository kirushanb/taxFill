...
const EditPackage = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      }else if(deletepackage.toLowerCase().replace(/\s/g, "")==='selfemployment'){
        const response = await axiosPrivate.delete(
          `https://tax.api.cyberozunu.com/api/SelfEmployment/${deletepackageId}`
        );
      }else if(deletepackage.toLowerCase().replace(/\s/g, "")==='pensionincome'){
        const response = await axiosPrivate.delete(
  ...
  return (
    <React.Fragment>
    <ToastContainer />
   {loading?<React.Fragment>{loading && <div className="EditPackage">
            ...
            <>
              ...
              <div className={`modal ${deleteModal?"is-active":""}`}>
                ...
                <div className="modal-content">
                  ...
                  <p className="title is-5">Are you sure to want to delete this package?</p>
                  <div className="delete-footer">
                  <button className="button is-danger" onClick={handleConfirmDelete}>Delete</button>
                  <button className="button is-warning" onClick={() => {setDeleteModal(modal=>!modal);
                    setDeletepackage('');
                    setDeletepackageId('');
                  }}>Cancel</button>
                  </div>
                ...
                <button
                  ...
                  aria-label="close"
                  onClick={() => {setDeleteModal(modal=>!modal);
                    setDeletepackage('');
                    setDeletepackageId('');
                  }}
                ></button>
